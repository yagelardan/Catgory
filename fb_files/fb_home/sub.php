<?php
	...
	if(isset($_GET['s']))
	{
		//sub name contains only letters and digits
		if(ctype_alnum($_GET['s']))
		{
			$subname=$_GET['s'];
		}
		else
  ...
  if($post_unique_like_count==0)
  {
	...
	if(isset($_POST['file']) && ($_POST['file']=='post'))
	{
		...
		if($chk_post_sub_name==1 && $chk_post_country_name==1)
		{
			...
			if($count_all_user_recent_posts>5)
			{
				//$txt="You posts too much - ".$txt ;
			...
			}else{header('Location: ?s='.$_GET['s'].'&al=2');;}
	...
	$result_color = mysql_query("SELECT * FROM categories WHERE cat_name='$subname' LIMIT 1");
	...
	elseif($subname=="finance")
	{
		$color_design="4caf50";
	}
	elseif($subname=="sport")
	{
		$color_design="ff7043";
	}
	*/
...
<title><?php echo $subname;?></title>
    ...
    <script>
		...
		var selected = document.getElementById("suboption");
	   ...
	   if(selectedVal != ""){
		...
		function time_get1()
		{
			...
			var selected = document.getElementById("suboptionphoto");
			var selectedVal = selected.options[selected.selectedIndex].value;
		   if(selectedVal != ""){
			  //do whatever
		   }else{alert('select category to the post');}
...
<body style="background-color:#fafafa; font-family: Calibri,Candara,Segoe,Segoe UI,Optima,Arial,sans-serif; ">
	<div id="wrapper">
		...
		<!-- Header -->
			<header id="header" style="background-color:rgba(255, 255, 255, 0.7); color:black; overflow: hidden; border-style: solid; border-bottom: solid #<?php echo $color_design;?>;">

<a href="#" class="logo"><img src="images/logo.jpg" alt="" style="height:100%;" /></a>
				<h1><a href="Home.php" style="font-size: 70%; float:left; color:#<?php echo $color_design;?>;"><?php echo $subname;?></a></h1>
				<nav class="links">
					<ul>
			...
			<section id="sidebar">
				...
				<!-- Intro -->
					<section id="intro">
					<header>
						<h2><?php echo ucfirst($subname);?></h2>
					</header>
						<script>
                                                       $("#postform").hide();
				...
				?>
					...
					<div id="postform" style="background-color:#fff;">	
						...
						<form method="post" name="posting_txt" onSubmit="return blank_post_check();" id="post_txt">
							...
							<div>
								<select style="background: transparent; width:50%; float:left;" name="subname" id="suboption"> 
						...
						<script>
							document.getElementById('suboption').value = '<?php echo $subname;?>';
						...
						<form method="post" enctype="multipart/form-data" name="posting_pic" style="display:none;" id="post_pic" onSubmit="return Img_check();">
								...
								<div >
								<select style="background: transparent; width:50%; float:left;" name="subname" id="suboptionphoto"> 
									<option value=""> Choose subject </option> 
									<?php
...
<p id="progressBar"></p>
												...
												<script>
							document.getElementById('suboptionphoto').value = '<?php echo $subname;?>';
				...
				<script src="lovebtn/js/index.js"></script>
					...
					<input type="hidden" id="sub_id" value="<?php echo $subname;?>">
					...
					var val = 0;
					var subval = document.getElementById("sub_id").value;
					var useridval = document.getElementById("user_id").value;
				...
				<!--
					<ul class="actions pagination">
						<?php
							...
							{
								$prv_page=1;
								echo '<li><a href="sub.php?s='.$subname.'&p='.$prv_page.'" class="disabled button big previous">Previous Page</a></li>';
							}
							else if($pagenum==2)
							{
								echo '<li><a href="sub.php?s='.$subname.'" class="button big previous">Previous Page</a></li>';
							}
							else
							{

							echo '<li><a href="sub.php?s='.$subname.'&p='.$prv_page.'" class="button big previous">Previous Page</a></li>';
							}
