...
//$que_post=mysql_query("select * from user_post where sub_id='$sub_name' order by post_id desc limit $no,10");
/* working
...
/* working - random
							...
							FROM user_post LEFT JOIN user_likes 
							ON user_post.post_id = user_likes.post_id
                                                        WHERE sub_id='$sub_name' AND user_post.country_id='$countryselect'
							GROUP BY user_post.post_id
...
*/
				...
				$beta=rand(5, 200) / 10;
							...
							FROM user_post LEFT JOIN user_likes 
							ON user_post.post_id = user_likes.post_id
                                                        WHERE sub_id='$sub_name' AND ((UNIX_TIMESTAMP()-user_post.post_unix_time)>100) And (user_post.country_id='$countryselect')
							GROUP BY user_post.post_id
							ORDER BY like_count desc limit $no,15";	
                                                        $que_post=mysql_query($result11);
...
?>
				...
				?>
					...
					?>
						...
						<img src="<?php echo get_youtube_image($url);?>" width="100%" onerror="this.src=''" height="300px" style="pointer-events: none;">
						</div>
						<?php
							}
